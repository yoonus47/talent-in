-- Both, not either/or: last_active_at_sydney/joined_at_sydney (3rd/4th
-- columns) go back to being readable text for quick scanning, and the
-- actual sortable timestamps move to the end as
-- last_active_at_sydney_raw/joined_at_sydney_raw — still Sydney local
-- time (same Australia/Sydney conversion, DST-aware), just left as a
-- real `timestamp` instead of formatted text, so clicking to sort in
-- Table Editor works correctly on those two columns specifically.
--
-- Reordering + re-adding a text-typed pair of columns both require a
-- drop/recreate again (same reasoning as 0035/0037/0038), with the
-- lockdown re-applied at the bottom as always.
