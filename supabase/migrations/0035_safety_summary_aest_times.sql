-- Display timestamps in the view as Australian east-coast local time
-- instead of raw UTC, for easier reading in the Table Editor. Sydney/
-- Melbourne switch between AEST (UTC+10) and AEDT (UTC+11) with daylight
-- saving (Oct-Apr) — `Australia/Sydney` tracks that automatically, so
-- these always match real local wall-clock time rather than staying
-- fixed at one offset. (If you're in Queensland, which never observes
-- daylight saving, swap this to `Australia/Brisbane` for a fixed AEST.)
--
-- `create or replace view` can't change an existing column's type, and
-- `at time zone` turns a timestamptz into a plain timestamp — so this
-- drops and recreates the view rather than replacing it in place. Drop
-- removes its privileges along with it, so the revoke at the bottom is
-- re-applied, not just left over from 0034.
...
select
  ...
  p.is_minor,
  p.grade,
  (p.created_at at time zone 'Australia/Sydney') as joined_at_aest,
  (p.last_active_at at time zone 'Australia/Sydney') as last_active_at_aest,
